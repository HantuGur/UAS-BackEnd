...
{
    /**
     * Menampilkan daftar semua pesanan beserta data customer dan item rincian pesanan.
     */
    ...
    /**
     * Menampilkan form untuk admin membuat pesanan baru (walk-in / kasir).
     */
    ...
    /**
     * Menyimpan pesanan baru dari admin (walk-in / kasir) ke database.
     */
    public function store(Request $request)
    {
        ...
        $order = DB::transaction(function () use ($request) {
            ...
            if ($request->customer_type === 'new') {
                ...
                if (empty($email)) {
                    // Buat email samaran unik jika kosong
        ...
        if ($request->wantsJson()) {
            ...
            // Jika bayar cash, langsung arahkan ke halaman kasir
            if ($request->payment_method === 'cash') {
                return redirect()->route('payments.create', ['order_id' => $order->id])
                    ->with('success', 'Pesanan berhasil dibuat. Silakan proses pembayaran tunai.');
            } elseif ($request->payment_method === 'qris') {
                // Buat payment pending untuk QRIS
                ...
                return redirect()->route('admin.orders.qris', $order->id);
            }
    ...
    /**
     * Menampilkan data detail pesanan tertentu beserta data customer dan rincian item.
     */
    ...
    /**
     * Menampilkan form untuk mengubah status pesanan tertentu.
     */
    ...
    /**
     * Memperbarui status pesanan (misalnya dari 'pending' menjadi 'completed').
     */
    ...
    /**
     * Menghapus pesanan tertentu beserta rincian itemnya (cascade delete).
     */
    ...
    /**
     * Menyaring pesanan berdasarkan status ('pending' atau 'completed').
     */
    ...
    // Menampilkan simulator QRIS untuk Kasir
    public function showQris($id)
    {
        $order = Order::findOrFail($id);
        $payment = \App\Models\Payment::where('order_id', $order->id)->first();
        return view('orders.qris', compact('order', 'payment'));
    }

    // Konfirmasi QRIS oleh Kasir
    public function confirmQris($id)
    {
        ...
        return redirect()->route('orders.show', $order->id)->with('success', 'Pembayaran QRIS berhasil dikonfirmasi kasir!');
    }
}